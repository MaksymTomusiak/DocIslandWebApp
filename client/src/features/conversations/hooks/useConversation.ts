...
export const useConversation = (baseURL: string) => {
    ...
    const getTokenWithTemplate = useCallback(() => getToken({ template: process.env.VITE_CLERK_JWT_TEMPLATE || '' }), [getToken]);
    const [conversations, setConversations] = useState<ConversationDto[]>([]);
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState<string | null>(null);

    const conversationApi = useMemo(
        () => new ConversationApi(baseURL, new AbortController().signal, getTokenWithTemplate),
    ...
    const createConversation = useCallback(async (file: File) => {
        try {
            setLoading(true);
            setError(null);
            const newConversation = await conversationApi.createConversation({ file });
